      ...
      async.series([
        function createMenuLinks(done) {
          we.db.models.cfmenu.findOne({
            where: { eventId: r.id, name: 'main' }
          }).then(function (menu) {
            we.db.models.cflink.bulkCreate([
              { href: '/event/1', text: 'Início', weight: 0, eventId: r.id, cfmenuId: menu.id  },
              { href: '/event/1#schedule', text: 'Programação', weight: 1,eventId: r.id, cfmenuId: menu.id },
              { href: '/event/1#speakers', text: 'Palestrantes', weight: 2,eventId: r.id, cfmenuId: menu.id },
              { href: '/event/1#ourLocation', text: 'Localização', weight: 3,eventId: r.id, cfmenuId: menu.id },
              { href: '/event/1/cfnews', text: 'Notícias', weight: 4,eventId: r.id, cfmenuId: menu.id },
            ]).then(function() {
              done();
            }).catch(done);
          });
        ...
        function createSocialMenuLinks(done) {
          we.db.models.cfmenu.findOne({
            where: { eventId: r.id, name: 'social' }
          }).then(function (menu) {
            we.db.models.cflink.bulkCreate([
              { href: '/event/1', text: '<i class="fa fa-youtube-play"></i>', weight: 0, eventId: r.id, cfmenuId: menu.id, class: 'btn btn-empty-inverse btn-lg' },
              { href: '/event/1', text: '<i class="fa fa fa-twitter"></i>', weight: 1,eventId: r.id, cfmenuId: menu.id, class: 'btn btn-empty-inverse btn-lg' },
              { href: '/event/1', text: '<i class="fa fa-facebook"></i>', weight: 2,eventId: r.id, cfmenuId: menu.id, class: 'btn btn-empty-inverse btn-lg' }
            ]).then(function() {
              done();
            }).catch(done);
          });
        ...
        function createKeynotes(done) {
          we.db.models.cfspeaker.bulkCreate([
            { name: 'Renato Janine', weight: 5, eventId: r.id, about: 'Ministro da Educação. Professor titular de Ética e Filosofia Política na Universidade de São Paulo, na qual se doutorou após defender mestrado na Sorbonne. Tem se dedicado à análise de temas como o caráter teatral da representação política, a ideia de revolução, a democracia, a república, a cultura política brasileira. Atualmente é professor-titular da cadeira de Ética e Filosofia Política da Faculdade de Filosofia, Letras e Ciências Humanas da Universidade de São Paulo.',
              highlighted: true  },
            { name: 'Heider Aurélio Pinto', weight: 5, eventId: r.id, about: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit.',
              highlighted: true },
            { name: 'Manuel Palacios', weight: 5, eventId: r.id, about: 'Secretário de educação básica do Ministério da Educação (MEC).',
              highlighted: true },
            { name: 'Mario Rovere', weight: 5, eventId: r.id, about: 'Professor da Universidad de Buenos Aires, Universidad Nacional de Rosario, Faculdade Latinoamericana de Ciencias Sociais (FLACSO).  Pesquisador da Universidad Nacional de La Matanza, Asociación Latinoamricana de Medicina Social ALAMES.',
              highlighted: true },
            { name: 'Naomar Almeida-Filho', weight: 5, eventId: r.id, about: 'Professor Titular de Epidemiologia no Instituto de Saúde Coletiva da UFBA. Reitor da Universidade Federal do Sul da Bahia.',
              highlighted: true },
            { name: 'Naomar Almeida-Filho', weight: 5, eventId: r.id, about: 'Professor de Novas Tecnologias na USP. Pesquisador de Projetos Educacionais Inovadores com metodologias ativas nas modalidades presencial e a distância.',
              highlighted: true },
            { name: 'Chao Lung Wen', weight: 5, eventId: r.id, about: 'Coordenador do Núcleo de Telemedicina e Telessaúde do Hospital das Clínicas da FMUSP. Coordenador do Projeto Homem Virtual e Mídias com impressão 3D.',
              highlighted: true },
            { name: 'Roberto Lent', weight: 5, eventId: r.id, about: 'Professor titular da Universidade Federal do Rio de Janeiro, diretor do Instituto de Ciências Biomédicas. É membro titular da Academia Brasileira de Ciências, membro do Conselho Técnico-Científico da CAPES - Ensino Básico, e presidente do Conselho Deliberativo do Instituto Ciência Hoje.',
              highlighted: true },
            { name: 'Isaac Roitman', weight: 5, eventId: r.id, about: 'Professor emérito da Universidade de Brasília, coordenador do Núcleo de Estudos do Futuro (n.Futuros/CEAM/UnB), presidente do Comitê Editorial da Revista Darcy/UnB e membro tiular de Academia Brasileira de Ciências.',
              highlighted: true  },
            { name: 'Kenneth Rochel de Camargo Jr', weight: 5, eventId: r.id, about: 'Professor adjunto do Instituto de Medicina Social da Universidade do Estado do Rio de Janeiro (IMS/UERJ), editor do American Journal of Public Health e editor da revista Physis. Membro da Comissão de Ciências Sociais e Humanas em Saúde da Abrasco.',
              highlighted: true },
            { name: 'Pierre Levy', weight: 4, eventId: r.id, about: 'Professor no Departamento de Hipermídia da Universidade de Paris-VIII. Dedicou sua vida profissional ao entendimento das implicações culturais e cognitivas das tecnologias digitais, com o objetivo de promover seus melhores usos sociais e estudar o fenômeno da inteligência coletiva. Ele é autor de vários livros sobre o assunto que foram traduzidos para mais de 12 línguas e estudados em diferentes universidades em todo o mundo. Atualmente, leciona no departamento de comunicação da Universidade de Ottawa (Canada), onde ele conduz pesquisas sobre Inteligência Coletiva.',
              highlighted: true },
          ]).then(function() {
            done();
          }).catch(done);
          ...
          we.db.models.widget.bulkCreate([
            {
              ...
              context: 'event-'+r.id,
              theme: 'we-theme-event-seminarioeducacao.fiocruz.br',
              weight: 8,
              type: 'we-cf-topics'
            },
            {
        ...
        function createCfTopics(done) {
          we.db.models.cftopic.bulkCreate([
            { title: 'Educação, Tecnologia e Sociedade: qual futuro queremos?', weight: 5, eventId: r.id },
            { title: 'Educação e Trabalho para o SUS: desafios de um projeto de país', weight: 6, eventId: r.id },
            { title: 'Formação para a Pesquisa e Inovação', weight: 7, eventId: r.id }
          ]).then(function() {
            done();
          }).catch(done);
        }
